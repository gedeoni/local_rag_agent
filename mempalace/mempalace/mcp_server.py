...
TOOLS = {
    ...
    "mempalace_diary_write": {
        "description": "WRITE a memory entry to your personal agent diary. Requires 'agent_name' and 'entry' (in AAAK format). This is NOT a search tool and does NOT take a 'q' or 'query' parameter.",
        "input_schema": {
            "type": "object",
            "properties": {
                "agent_name": {
...
def handle_request(request):
    ...
    if method == "initialize":
        ...
        # --- Parameter Aliasing ---
        # Some models (like llama3.2) stubbornly send 'q' instead of 'query' 
        # for search tools. If 'q' is present and 'query' is missing but expected, map it.
        if "q" in tool_args and "query" in schema_props and "query" not in tool_args:
            logger.info(f"Aliasing 'q' -> 'query' for tool '{tool_name}'")
            tool_args["query"] = tool_args.pop("q")

        filtered_args = {}
        for key, value in list(tool_args.items()):
            if key not in schema_props:
                logger.warning(f"Ignoring unexpected argument '{key}' for tool '{tool_name}'")
                continue
            
            prop_schema = schema_props.get(key, {})
            declared_type = prop_schema.get("type")
            if declared_type == "integer" and not isinstance(value, int):
                filtered_args[key] = int(value)
            elif declared_type == "number" and not isinstance(value, (int, float)):
                filtered_args[key] = float(value)
            else:
